...
from ..core.models import ExperimentResults, DeliberationResponse, FeedbackResponse
...
class DataExporter:
    ...
    def export_csv(self, results: ExperimentResults) -> Dict[str, str]:
        ...
        # 5. Agent memories CSV (if available)
        if results.agent_memories:
            memories_path = self._export_agent_memories_csv(results)
            csv_files["agent_memories"] = str(memories_path)
        
        # 6. Speaking orders CSV (if available)
        if results.speaking_orders:
            speaking_path = self._export_speaking_orders_csv(results)
            csv_files["speaking_orders"] = str(speaking_path)
    ...
    def _export_transcript_csv(self, results: ExperimentResults) -> Path:
        ...
        with open(filepath, 'w', newline='', encoding='utf-8') as f:
            ...
            writer.writerow([
                ...
                "Principle_Name", "Message", "Timestamp"
            ])
    ...
    def _export_feedback_csv(self, results: ExperimentResults) -> Path:
        ...
        with open(filepath, 'w', newline='', encoding='utf-8') as f:
            ...
            writer.writerow([
                ...
                "Would_Choose_Again", "Alternative_Preference", 
                "Reasoning", "Timestamp"
            ])
    ...
    def _export_agent_memories_csv(self, results: ExperimentResults) -> Path:
        """Export agent private memories as CSV."""
        filename = f"{results.experiment_id}_agent_memories.csv"
        filepath = self.output_dir / filename
        ...
        with open(filepath, 'w', newline='', encoding='utf-8') as f:
            ...
            # Header
            writer.writerow([
                "Agent_ID", "Round", "Speaking_Position", "Situation_Assessment", 
                "Other_Agents_Analysis", "Strategy_Update", "Timestamp"
            ])
            
            # Data rows
            for agent_memory in results.agent_memories:
                for memory_entry in agent_memory.memory_entries:
                    writer.writerow([
                        agent_memory.agent_id,
                        memory_entry.round_number,
                        memory_entry.speaking_position,
                        memory_entry.situation_assessment.replace('\n', ' ').replace('\r', ''),
                        memory_entry.other_agents_analysis.replace('\n', ' ').replace('\r', ''),
                        memory_entry.strategy_update.replace('\n', ' ').replace('\r', ''),
                        memory_entry.timestamp.isoformat()
                    ])
        
        return filepath
    
    def _export_speaking_orders_csv(self, results: ExperimentResults) -> Path:
        """Export speaking orders for each round as CSV."""
        filename = f"{results.experiment_id}_speaking_orders.csv"
        filepath = self.output_dir / filename
        
        with open(filepath, 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            
            # Find maximum number of agents for header
            max_agents = max(len(order) for order in results.speaking_orders) if results.speaking_orders else 0
            
            # Header
            header = ["Round"] + [f"Position_{i+1}" for i in range(max_agents)]
            writer.writerow(header)
            
            # Data rows
            for round_num, speaking_order in enumerate(results.speaking_orders):
                round_name = "Initial" if round_num == 0 else f"Round {round_num}"
                row = [round_name] + speaking_order + [""] * (max_agents - len(speaking_order))
                writer.writerow(row)
        ...
        with open(filepath, 'w', encoding='utf-8') as f:
            ...
            for round_num in sorted(rounds.keys()):
                ...
                for response in responses:
                    ...
                    f.write(f"{response.public_message}\n\n")
